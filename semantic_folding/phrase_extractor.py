    ...
    with open(corpus_path, 'r', encoding='utf-8') as f:
        for i, line in tqdm(enumerate(f)):
            ...
            try:
                ...
                logger.debug(f"[CORPUS] Line {i} skipped — parse error")
                continue
                
            logger.debug(f"[CORPUS] Line {i} | ctx='{ctx_id}' | text= Ar: '{arabic_text[:60]}...' En: '{english_text[:60]}...'")
            ...
            if not valid_sub_phrases:
                logger.debug(f"[CORPUS] Line {i} | no valid sub phrases — skipping expansion")
                continue
...
def extract_raw_phrases_spacy(doc) -> list[str]:
    ...
    NOMINAL_DEPS = {'nsubj', 'dobj', 'pobj', 'attr', 'ROOT', 'pcomp', 'acl', 'advcl'} ### Prevent verb deletion example questioning
    ...
    for token in doc:
        ...
        if any(c.dep_ in ('nsubj', 'nsubjpass') for c in token.children):
            continue
        # if any(c.dep_ in ('dobj', 'attr', 'oprd') for c in token.children): ### before delete phrase example "questioning"
        #     continue
        gov = token.head
