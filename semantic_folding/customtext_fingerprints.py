...
Aggregates phrase-level sparse fingerprints (Step 4) into custom-text-level
...
def build_customtext_fingerprints(*args, **kwargs):
    """Wrapper around build_fingerprints with Step 6 defaults."""
    return build_fingerprints(*args, step_label="6", file_prefix="doc", **kwargs)
...
def main():
    ...
    parser.add_argument("--output", type=Path, required=True, help="Output directory for customtext fingerprints")
    ...
    fp_matrix, doc_index_map, stats, doc_norms = build_customtext_fingerprints(
    ...
    logger.info("Check: Step 6 complete")
