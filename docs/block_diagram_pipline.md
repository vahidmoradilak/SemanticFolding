...
flowchart TD
    ...
    subgraph STEP1["🔧 Step 1\nPhrase Extraction (phrases)"]
        B1["Tokenization (terms)"]
        B2["Language Detection"]
        B3["English: spaCy\nen_core_web_sm"]
        B4["Arabic: NLTK\nPOS-based extraction"]
        B5["Lemmatization and Normalization"]
        B6["POS Filtering"]
        B7["Noun Chunks and Compound Detection"]
        B8["Bigram / Trigram Generation"]
        B9["Phrase Validation"]
        B10["Output:\nextracted_phrases/"]
    end
...
## 2. Extraction Phrase from English vs Arabic text

```mermaid
flowchart TD
    subgraph COMMON["Shared Design"]
        S["POS-pattern strategy\nfor both languages"]
    end
...
## 5. Fusion Methods (SF + SPLADE) and Final Results
...
flowchart TD
    ...
    subgraph TAKEAWAY["Takeaways"]
        ...
        T3["SF+SPLADE beats BM25 on\nBelebele, NarrativeQA, SciFact, SciDocs"]
    end
