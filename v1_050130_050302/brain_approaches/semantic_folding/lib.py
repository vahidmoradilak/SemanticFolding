...
nltk.data.path.insert(0, 'D:\\darsi\\ارشد\\Thesis\\Dr.Banaie\\code050302\\nltk_data')
...
def expand_phrases(
    ...
    for raw_phrase in phrases:
        ...
        for candidate in candidates:
            ...
            if detect_language(candidate) == "en":
            # if detect_language(candidate) != "ff":
                ...
                normEn = normalize_phrase(candidate, remove_verbs=True)
                if not normEn:
                ...
                if ' ' not in normEn and filter_generic and is_generic_word(normEn, min_word_length):
                    logger.debug(
                        f"  [GENERIC DROP] '{normEn}' (from '{candidate}') "
                ...
                # baraye kalamat arabi nyaz be palayesh bishtari vojood dare !!!!!
                norm = normalize_arabic_phrase(candidate)
                # if len(norm.split()) == 1 and len(norm) < 3:
                    # continue #for delete arabic tokens
                if not norm:
                    continue
            # ── step 5: accumulate ────────────────────────────────────────────
